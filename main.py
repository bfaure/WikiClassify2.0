...
def save_related(model_name):
    ...
    if not os.path.exists(word2vec_directory+'/'+model_name):
        if model_name == 'words':
            ...
            encoder.build(features=200, context_window=50, min_count=3, sample=1e-5, negative=5)
            encoder.train(epochs=50)
        elif model_name == 'cited_authors':
            encoder.build(features=20, context_window=50, min_count=2)
            encoder.train(epochs=100)
        elif model_name == 'cited_domains':
            encoder.build(features=80, context_window=50, min_count=2)
            encoder.train(epochs=100)
        encoder.save()
    else:
        self.load()
